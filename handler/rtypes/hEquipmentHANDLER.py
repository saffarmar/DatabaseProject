...
from dao.rtypes.hEquipmentDAO import HeavyEquipmentDao
...
class HeavyEquipmentHandler:
    ...
    def insertAvailableResource(self, form):
        ...
        if len(form) != 10:
            ...
            weight = form['weight']
            age = form['age']
            if erequirement and brand and etype and weight and age and uid and kind and amount and price and restime:
                daor = ResourceDao()
                daoa = AvailableDao()
                rid = daor.addResource(uid, kind)
                daoa.addAvailable(rid, uid, amount, price, restime)
                dao = HeavyEquipmentDao()
                dao.addHeavyEquipment(rid, erequirement, etype, weight, age, brand)
    ...
    def insertRequestedResource(self, form):
        ...
        if len(form) != 10:
            ...
            weight = form['weight']
            age = form['age']
            if erequirement and brand and etype and weight and age and uid and kind and amount and restime:
                daor = ResourceDao()
                daoa = RequestedDao()
                rid = daor.addResource(uid, kind)
