...
from dao.rtypes.clothingDAO import ClothingDao
...
class ClothingHandler:
    ...
    def insertAvailableResource(self, form):
        ...
        if len(form) != 9:
            ...
            if gender and brand and size and material and uid and kind and amount and price and restime:
                daor = ResourceDao()
                daoa = AvailableDao()
                rid = daor.addResource(uid, kind)
                daoa.addAvailable(rid, uid, amount, price, restime)
                dao = ClothingDao()
                dao.addClothing(gender, brand, material, size)
    ...
    def insertRequestedResource(self, form):
        ...
        if len(form) != 9:
            ...
            size = form['size']
            material = form['material']
            if gender and brand and size and material and uid and kind and amount and restime:
                daor = ResourceDao()
                daoa = RequestedDao()
                rid = daor.addResource(uid, kind)
