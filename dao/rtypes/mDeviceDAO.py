...
class MedicalDeviceDao:
    ...
    def addMedicalDevice(self, rid, erequirement, weight):
        cursor = self.conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        query = "INSERT INTO medicalDevices(rid, erequirement, weight) VALUES (%s, %s, %s);"
        cursor.execute(query, (rid, erequirement, weight))
